            ...
            <tr className={isBulkEditing ? 'bg-slate-50' : 'bg-blue-50/50'}>
                <td className={`px-6 py-4 whitespace-nowrap sticky left-0 ${isBulkEditing ? 'bg-slate-50' : 'bg-blue-50/50'}`}>
                    <div className="flex gap-2">
                        <input type="text" value={data.first_name} onChange={(e) => handleFieldChange('first_name', e.target.value)} className="input-neo w-32" />
                        <input type="text" value={data.last_name} onChange={(e) => handleFieldChange('last_name', e.target.value)} className="input-neo w-32" />
                    </div>
                </td>
                <td className="px-6 py-4 whitespace-nowrap text-center">
                    <input type="number" min="1" max="10" value={data.skill} onChange={(e) => handleFieldChange('skill', parseInt(e.target.value))} className="input-neo w-20 text-center" />
                </td>
                <td className="px-6 py-4 whitespace-nowrap text-center">
                    <select value={data.is_defense ? "defense" : "forward"} onChange={(e) => handleFieldChange('is_defense', e.target.value === "defense")} className="input-neo w-28">
                        <option value="forward">Forward</option>
                        <option value="defense">Defense</option>
                    </select>
                </td>
                <td className="px-6 py-4 whitespace-nowrap text-center">
                    <select value={data.is_attending ? "yes" : "no"} onChange={(e) => handleFieldChange('is_attending', e.target.value === "yes")} className="input-neo w-24">
    ...
    return (
        <tr className="hover:bg-slate-50/80">
            <td className="px-6 py-4 whitespace-nowrap sticky left-0 bg-white">
                <div className="text-sm font-medium text-slate-900">{player.first_name} {player.last_name}</div>
            </td>
            <td className="px-6 py-4 whitespace-nowrap text-center">
                <span className="badge-neo bg-blue-50 text-blue-700">{player.skill}</span>
            </td>
            <td className="px-6 py-4 whitespace-nowrap text-center">
