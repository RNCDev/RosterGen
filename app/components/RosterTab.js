...
export const RosterTab = ({ teams, generateRosters, players, loading }) => {
  const hasTeams = teams && teams.red && teams.red.forwards && teams.red.forwards.length > 0 && teams.white && teams.white.forwards && teams.white.forwards.length > 0;
    ...
    <div className="roster-tab">
      {hasTeams ? (
          ...
          <div className="red-team team-card bg-red-50 p-4 rounded shadow">
            ...
            <div className="forwards mb-4">
              ...
              {teams.red.forwards.map((player) => (
                <div key={player.id} className="player-item bg-red-100 rounded px-3 py-2 mb-1 flex justify-between items-center">
                  <span>
          ...
          <div className="white-team team-card bg-gray-50 p-4 rounded shadow">
            ...
            <div className="forwards mb-4">
              ...
              {teams.white.forwards.map((player) => (
                <div key={player.id} className="player-item bg-gray-100 rounded px-3 py-2 mb-1 flex justify-between items-center">
                  <span>
                    {player.first_name} {player.last_name}
                  </span>
        ...
        <div className="no-teams-generated text-center py-8">
          <button onClick={generateRosters} className="bg-blue-500 text-white px-6 py-3 rounded hover:bg-blue-600 inline-flex items-center" disabled={players.length === 0 || loading}>
            <ArrowLeftRight className="mr-2" /> {loading ? "Generating..." : "Generate Teams"}
          </button>
          {players.length === 0 && <p className="text-red-500 mt-2">Please upload players first before generating teams.</p>}
        </div>
      )}
    </div>
  );
